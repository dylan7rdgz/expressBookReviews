...
let users = [{"username": "u_name_1", password: "Hello!23"}];
...
regd_users.put("/auth/review/:isbn", (req, res) => {
  ...
  if (0<ISBNno && ISBNno<11) {
    ...
    books[ISBNno].reviews[req.session.authorization.username] = req.query.review;
    req.query.review;
...
regd_users.delete("/auth/review/:isbn", (req, res) => {
  ...
  if (0<ISBNno && ISBNno<11) {
    console.log(req.session);
    books[ISBNno].reviews[req.session.authorization.username] = null;
    req.query.review;
    return res.status(200).json(books[ISBNno]);
  }
  console.log("no book with the requested isbn no.")
